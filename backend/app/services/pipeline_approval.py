...
async def enqueue_pending_decision_notifications(
    ...
    target_ids = (
        await db.scalars(
            ...
            .where(
                DeliveryTarget.organization_id == task.organization_id,
                DeliveryTarget.provider == "feishu",
                DeliveryTarget.is_active.is_(True),
                RoutingRule.organization_id == task.organization_id,
    ...
    for target_id in target_ids:
        await enqueue_channel_delivery(
            ...
            idempotency_key=f"decision-pending:{decision.id}:feishu:{target_id}",
        )
