...
async def _feishu_route_for_decision(decision_id: int) -> int:
    async with SessionLocal() as db:
        ...
        target = DeliveryTarget(
            ...
            provider="feishu",
            external_conversation_id=f"oc-reminder-{uuid4().hex[:8]}",
            is_active=True,
        )
        db.add(target)
        await db.flush()
        identity = ChannelIdentity(
            organization_id=decision.organization_id,
            provider="feishu",
            external_user_id=f"ou-reminder-{uuid4().hex[:8]}",
...
async def test_pending_decision_reminder_is_enqueued_once_after_threshold() -> None:
    ...
    target_id = await _feishu_route_for_decision(decision_id)
    try:
            ...
            (
                await db.scalars(
                    select(DeliveryOutbox).where(
                        ...
                        == f"decision-reminder:{decision_id}:feishu:{target_id}"
                    )
                )
            ).all()
        )
    ...
    assert delivery_rows[0].event_type == "pipeline.decision.reminder"
    assert delivery_rows[0].payload == {
...
async def test_pending_decision_escalation_is_enqueued_once_without_terminal_status() -> None:
        ...
        delivery_rows = list(
            (
                await db.scalars(
                    select(DeliveryOutbox).where(
                        DeliveryOutbox.idempotency_key
                        == f"decision-escalation:{decision_id}:feishu:{target_id}"
                    )
                )
            ).all()
        )
    ...
    assert count == 1
    assert len(delivery_rows) == 1
    assert delivery_rows[0].event_type == "pipeline.decision.escalation"
    assert delivery_rows[0].payload == {
        "decision_id": decision_id,
        "task_id": decision.task_id,
        "status": "pending",
