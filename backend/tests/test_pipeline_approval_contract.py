...
async def test_creator_approval_still_rejects_unknown_escalation_role(
    ...
    response = await client.post(
        ...
        json={
            "confirmed": True,
            "title": "Creator approval validation",
            "prompt": "生成审批策略校验报告",
            "task_type": "general",
            "schedule": None,
            "timezone": "Asia/Shanghai",
            ...
            "approval_escalation_role_name": "role-that-does-not-exist",
        },
    )

    assert response.status_code == 422
    assert "escalation role does not exist" in response.text
