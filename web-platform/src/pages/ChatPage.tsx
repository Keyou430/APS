...
function renderPlatformAction(
  ...
  onConfirm: () => void,
  onCancel: () => void,
  onDraftChange: (patch: PipelineTaskRequest) => void,
) {
  ...
  return <div className="chat-evidence-notice" data-testid="chat-platform-action" role="status">
    ...
    {action.draft ? <div className="chat-platform-draft">
      <PipelineDraftEditor disabled={action.pending} draft={action.draft} onChange={onDraftChange} />
...
export function ChatPage({ cache, organizationId, pipeline, service, stream }: ChatPageProps) {
  ...
  const stopPromisesRef = useRef(new Map<string, Promise<unknown>>());
  const selectedIdRef = useRef<string | null>(null);
  const sessionGenerationRef = useRef(0);
  const cacheKey = useMemo(() => ["chat", "sessions"], []);
  const sending = selectedId ? sendingSessionIds.includes(selectedId) : false;

  const loadMessages = useCallback(async (sessionId: string, generation: number) => {
    const response = await service.getMessages(sessionId);
    if (selectedIdRef.current !== sessionId || sessionGenerationRef.current !== generation) return;
  ...
  const loadSessions = useCallback(async () => {
    ...
    } catch (error) { if (isCurrent()) { setStatus(errorStatus(error) === 403 ? "forbidden" : "error"); setErrorMessage(messageForError(error)); setSessions([]); setMessages([]); } }
  ...
  async function selectSession(sessionId: string) {
    ...
    setSelectedId(sessionId); setMessages([]); setErrorMessage(null); setStatus("loading");
    try { await loadMessages(sessionId, generation); if (sessionGenerationRef.current === generation) setStatus("success"); } catch (error) { if (sessionGenerationRef.current === generation) { setStatus("error"); setErrorMessage(messageForError(error)); } }
  }
  async function createSession() {
    ...
    try {
      ...
      const next = [...sessions, created]; setSessions(next); sessionGenerationRef.current += 1; selectedIdRef.current = created.id; setSelectedId(created.id); setMessages([]); setStatus("success");
  ...
  async function sendMessage(contentOverride?: string, clientMessageId?: string) {
    const content = (contentOverride ?? composer).trim(); if (!content || !selectedId || sendingSessionsRef.current.has(selectedId)) return;
    const runSessionId = selectedId;
    const streamGeneration = sessionGenerationRef.current;
    ...
    sendingSessionsRef.current.add(runSessionId);
    setSendingSessionIds(Array.from(sendingSessionsRef.current));
    setComposer(""); setErrorMessage(null); setMessages((current) => [...current, { id: `local-${userMessageId}`, role: "user", content, clientMessageId: userMessageId }, assistant]);
    const controller = new AbortController();
    abortControllersRef.current.set(runSessionId, controller);
    activeRunIdsRef.current.delete(runSessionId);
    stopPromisesRef.current.delete(runSessionId);
    const isCurrentStream = () => selectedIdRef.current === runSessionId && sessionGenerationRef.current === streamGeneration;
    try {
      const response = await stream.sendMessageStream(runSessionId, { content, attachments: [], client_message_id: userMessageId, links: [], metadata: { mode: "auto", command_mode: true } }, { signal: controller.signal });
      let pending = "";
      const apply = (event: string, data: unknown) => {
        const value = asObject(data);
        if (event === "run.created" && abortControllersRef.current.get(runSessionId) === controller) {
          const runId = readString(value.run_id);
          if (runId) activeRunIdsRef.current.set(runSessionId, runId);
        }
      ...
      if (isCurrentStream()) setMessages((current) => current.map((message) => message.id === assistant.id && message.status === "streaming" ? { ...message, status: "interrupted", content: message.content || "生成中断，请稍后重试。" } : message));
    } catch (error) { if (isCurrentStream()) { const message = messageForSendError(error); setMessages((current) => current.map((item) => item.id === assistant.id ? { ...item, status: controller.signal.aborted ? "interrupted" : "failed", content: item.content || message } : item)); setErrorMessage(message); } }
    finally {
      await stopPromisesRef.current.get(runSessionId);
      if (abortControllersRef.current.get(runSessionId) === controller) {
        abortControllersRef.current.delete(runSessionId);
        activeRunIdsRef.current.delete(runSessionId);
        stopPromisesRef.current.delete(runSessionId);
        sendingSessionsRef.current.delete(runSessionId);
        setSendingSessionIds(Array.from(sendingSessionsRef.current));
      }
    }
  }
  function stopGeneration() {
    ...
    if (sessionId && sendingSessionsRef.current.has(sessionId) && !stopPromisesRef.current.has(sessionId)) {
      stopPromisesRef.current.set(sessionId, service.stopRun(sessionId, activeRunIdsRef.current.get(sessionId) || "active").catch(() => undefined));
    }
    if (sessionId) abortControllersRef.current.get(sessionId)?.abort();
  }
  ...
  function updatePlatformDraft(messageId: string, patch: PipelineTaskRequest) {
    setMessages((current) => current.map((message) => message.id === messageId && message.platformAction?.draft
      ? { ...message, platformAction: { ...message.platformAction, draft: { ...message.platformAction.draft, ...patch } } }
      : message));
  }
  ...
  return <main aria-labelledby="chat-title" className="page-view chat-page">
    ...
    <section aria-label="会话工作台" className="chat-shell">
      ...
      <section aria-label="消息记录" className="chat-transcript"><h2>{sessions.find((session) => session.id === selectedId)?.title ?? "消息记录"}</h2>{messages.length ? messages.map((message, index) => { const previous = messages[index - 1]; const freshnessNotice = message.role === "assistant" && previous?.role === "user" ? getFreshnessNotice({ userContent: previous.content, answer: { content: message.content, status: message.status, webEvidence: message.webEvidence }, webSearchUsed: message.webSearchUsed, webSearchFailed: message.webSearchFailed }) : ""; return <article key={message.id} data-role={message.role}><strong>{message.role === "user" ? "我" : "AI"}</strong>{message.role === "assistant" ? <div dangerouslySetInnerHTML={{ __html: renderSafeAssistantMarkdown(message.content || (message.status === "streaming" ? "正在生成..." : "")) }} /> : <p>{message.content}</p>}{message.role === "assistant" ? renderPlatformAction(message.platformAction, () => void confirmPlatformAction(message.id), () => cancelPlatformAction(message.id), (patch) => updatePlatformDraft(message.id, patch)) : null}{message.role === "assistant" ? renderWebEvidence(message) : null}{freshnessNotice ? <div className="chat-evidence-notice" data-testid="freshness-evidence-notice" role="status">{freshnessNotice}</div> : null}{message.status && message.role === "assistant" ? <small>{message.status === "streaming" ? "生成中" : message.status === "completed" ? "已完成" : message.status === "interrupted" ? "已中断" : "失败"}</small> : null}{message.status && ["failed", "interrupted"].includes(message.status) ? <button onClick={() => void retryMessage(message)} type="button">重试</button> : null}</article>; }) : <p>选择会话后开始对话。</p>}</section>
    </section>
