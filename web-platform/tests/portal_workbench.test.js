...
test("portal workbench refinement contracts", async () => {
  ...
  // Workspace navigation and task deadline affordances.
  const sidebarOrder = [
    'data-scroll-target="cockpitKpiGrid"',
    'data-scroll-target="cockpit-decisions"',
    'data-scroll-target="cockpit-tasks"',
    'data-scroll-target="cockpit-docs"',
    'data-scroll-target="cockpit-shortcuts"',
  ];
  for (let index = 1; index < sidebarOrder.length; index += 1) {
    assert.ok(
      htmlSource.indexOf(sidebarOrder[index - 1]) < htmlSource.indexOf(sidebarOrder[index]),
      "workspace sidebar items must follow the requested order",
    );
  }
  assert.ok(
    htmlSource.includes('<div class="task-deadline-group">') &&
      htmlSource.indexOf('<span class="task-deadline-label">截止时间</span>') <
        htmlSource.indexOf('id="taskTag"') &&
      htmlSource.indexOf('id="taskTag"') < htmlSource.indexOf('id="taskTime"') &&
      htmlSource.includes('id="taskTime" aria-label="截止时间"'),
    "task time controls must be grouped under a label placed before both fields",
  );

  // Knowledge upload and experience-domain creation affordances.
  assert.ok(
    htmlSource.includes('<div class="inline-field">') &&
      htmlSource.includes('id="aiCreateCollectionButton"'),
    "knowledge upload folder controls must share one inline field",
  );
  assert.ok(
    cssSource.includes(".inline-field") && cssSource.includes(".inline-field select"),
    "knowledge upload folder controls need responsive inline layout styles",
  );
  const methodsViewBlock = appSource.slice(
    appSource.indexOf("function renderAiMethodsView"),
    appSource.indexOf("function renderAiSkillsView"),
  );
  assert.ok(
    /if \(domains\.length\)[\s\S]*id=\\?"aiAddMethod/.test(methodsViewBlock),
    "new experience method must only be offered after a domain exists",
  );
